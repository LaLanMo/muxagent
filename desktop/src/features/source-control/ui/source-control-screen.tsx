...
export function SourceControlScreen({
  ...
  return (
    ...
    >
      <section className="source-control" data-testid="source-control-screen">
                  ...
                  {workspaceOptions.map((workspace) => (
                    <button
                      className={`source-control__workspace-option${
                        workspace.label === workspaceLabel ? " is-active" : ""
                      }`}
                      key={workspace.id}
                    ...
                    >
                      ...
                      <span>{workspace.label === workspaceLabel ? "active" : "switch"}</span>
                    </button>
                  ))}
                </div>
              ) : null}
            ...
            {checkoutItems.map((entry) => (
              ...
              >
                <button
                  ...
                  data-testid={`source-control-checkout-${entry.label.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}`}
                  onClick={() => {
                    onSelectCheckout(entry.path);
                    ...
                    {entry.taskSummary.total > 0 ? (
                      <div className="source-control__group">
                        ...
                        {entry.taskSummary.rows.map((task) => (
                          <Link className="source-control__task-row" key={task.id} to={task.href}>
                              ...
                              {section.rows.map((fileRow) => (
                                <button
                                  className={`source-control__file-row${
                                    selectedFileId === fileRow.id ? " is-selected" : ""
                                  }`}
                                  key={fileRow.id}
                                  onClick={() => onSelectFile(fileRow.id)}
                                  type="button"
                                >
              ...
              <div className="source-control__file-detail-copy">
                ...
                <p>
                  {selectedFile.origPath
                    ? `Renamed from ${selectedFile.origPath}`
                    : "Change detail is loaded from checkout metadata in the app-server."}
                </p>
                <div className="source-control__file-detail-meta">
                  <span>{selectedCheckoutDetail?.branch ?? "detached"}</span>
                  <span>{selectedCheckoutDetail?.path ?? workspacePath ?? "workspace"}</span>
                </div>
              </div>
            </div>
          ) : (

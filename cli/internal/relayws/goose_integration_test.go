...
const (
	gooseModeAutoID         = "auto"
	gooseModeApproveID      = "approve"
	gooseModeSmartApproveID = "smart_approve"
)

func TestGooseRemoteControlEndToEnd(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping real Goose integration test in short mode")
	}
	if os.Getenv("MUXAGENT_E2E_GOOSE") == "" {
		t.Skip("set MUXAGENT_E2E_GOOSE=1 to run the real Goose integration test")
	}
	if _, err := exec.LookPath("goose"); err != nil {
		t.Skipf("goose not found on PATH: %v", err)
	}
	...
	require.True(t, ok)
	gooseRuntime := findRuntimeInfo(runtimeList.Runtimes, string(config.RuntimeGoose))
	require.NotNil(t, gooseRuntime)
	require.Equal(t, "Goose", gooseRuntime.Label)
	require.Equal(t, gooseModeAutoID, currentConfigValue(gooseRuntime.ConfigOptions, "mode"))

	createAny, errStr := client.rpcCreateSession(ctx, appwire.CreateSessionParams{
		CWD:            workdir,
		Runtime:        string(config.RuntimeGoose),
		PermissionMode: gooseModeSmartApproveID,
	})
	require.Empty(t, errStr)
	createResp, ok := createAny.(appwire.SessionCreateResult)
	require.True(t, ok)
	require.Equal(t, string(config.RuntimeGoose), createResp.App.Runtime)
	require.Equal(t, gooseModeSmartApproveID, currentConfigValue(createResp.ACP.ConfigOptions, "mode"))
	...
	if targetModel == originalModel {
		t.Skip("Goose ACP did not expose an alternate model to verify switching")
	}
	...
	_, errStr = client.rpcSetMode(ctx, appwire.SetModeParams{
		SessionID:      sessionID,
		PermissionMode: gooseModeApproveID,
	})
	require.Empty(t, errStr)
	modeEvent := waitForRuntimeEvent(t, rt.Events(), 30*time.Second, func(ev appwire.Event) bool {
		return ev.Type == appwire.EventModeChanged &&
			ev.SessionID == sessionID &&
			ev.ModeChanged != nil &&
			ev.ModeChanged.App.CurrentModeID == gooseModeApproveID
	})
	require.Equal(t, gooseModeApproveID, modeEvent.ModeChanged.App.CurrentModeID)
	...
	require.True(t, ok)
	require.Equal(t, string(config.RuntimeGoose), loadResp.App.Runtime)
	require.Equal(t, gooseModeSmartApproveID, currentConfigValue(loadResp.ACP.ConfigOptions, "mode"))
	...
	require.Equal(t, string(config.RuntimeGoose), resolveResp.Sessions[0].Runtime)
	require.Equal(t, workdir, resolveResp.Sessions[0].CWD)
	require.Equal(t, gooseModeSmartApproveID, currentConfigValue(resolveResp.Sessions[0].ConfigOptions, "mode"))

...
// rpcSessionGitStatus returns a git status snapshot for the session's cwd.
// The session identifies a remote device workspace; it is not necessarily a
// git repo, hence we return reachable=false + a reason rather than erroring.

...
import (
	...
	"github.com/LaLanMo/muxagent/cli/internal/geminisession"
	"github.com/LaLanMo/muxagent/cli/internal/goosesession"
...
func NewRegistry() *Registry {
	...
	r.Register(config.RuntimeGemini, ResolverFunc(resolveGeminiLocalSession))
	r.Register(config.RuntimeGoose, ResolverFunc(resolveGooseLocalSession))
...
func resolveGeminiLocalSession(sessionID string) (Metadata, error) {
	meta, err := geminisession.ResolveLocalSession(sessionID)
	if err != nil {
		if errors.Is(err, geminisession.ErrSessionNotFound) {
			return Metadata{}, fmt.Errorf("%w: %v", ErrSessionNotFound, err)
		}
		return Metadata{}, err
	}
...
func resolveGooseLocalSession(sessionID string) (Metadata, error) {
	meta, err := goosesession.ResolveLocalSession(sessionID)
	if err != nil {
		if errors.Is(err, goosesession.ErrSessionNotFound) {
			return Metadata{}, fmt.Errorf("%w: %v", ErrSessionNotFound, err)
		}
		return Metadata{}, err
	}
...
func resolveOpenCodeLocalSession(sessionID string) (Metadata, error) {
	meta, err := opencodesession.ResolveLocalSession(sessionID)
	if err != nil {
		if errors.Is(err, opencodesession.ErrSessionNotFound) {
			return Metadata{}, fmt.Errorf("%w: %v", ErrSessionNotFound, err)
		}
		return Metadata{}, err
	}

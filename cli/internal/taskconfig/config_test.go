...
func TestEmbeddedDefaultPlanningPromptsAllowReadCommands(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			contains: []string{
				"Read-only investigation is always allowed.",
				"Your only allowed writes are review artifacts under {{ARTIFACT_DIR}}",
			},
			excludes: []string{
	...
	tests := []struct {
	...
	}{
		{
			...
			contains: []string{
				...
				"Treat the plan as an outcome contract",
			},
			excludes: []string{
				"Task\n```\n{{TASK_DESCRIPTION}}\n```",
				"{{WORKFLOW_CONTEXT}}",
		...
		},
		{
			...
			contains: []string{
				...
				"{{WORKFLOW_CONTEXT_XML}}",
				"{{CLARIFICATION_CONTEXT_XML}}",
				"There is no human approval step in this workflow.",
		...
		},
		{
			...
			contains: []string{
				...
				"wave goal, done definition, required checks, constraints, and out-of-scope boundaries",
				"Wave goal status.",
			},
			excludes: []string{
				"Task\n```\n{{TASK_DESCRIPTION}}\n```",
...
func TestEmbeddedSingleRunPromptUsesMinimalDirectExecutionContract(t *testing.T) {
	...
	contains := []string{
		...
		"Write extra artifacts under {{ARTIFACT_DIR}} only when they add useful detail beyond `summary`.",
		"Put the primary human-facing result in `summary`.",
		"Put any extra artifact paths in `file_paths`.",
	}
	excludes := []string{
		"Task\n```\n{{TASK_DESCRIPTION}}\n```",
